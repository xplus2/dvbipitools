...
# asymmetric jitter: link A gets wide, correlated delay variance, link B stays clean
net_setup $PORTA delay 60ms 40ms 25%
...
cmp -s "$fixture" "$out" || fail "dipisrt: broadcast did not survive asymmetric jitter on one bonded link (see $WORK/send.log, $WORK/recv.log)"
