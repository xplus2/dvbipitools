...
START_TEST(rtx_build_round_trips_through_rtx_parse) {
  ...
  n = rtx_build(&seq, 0x11223344u, 99, 0xAABBCCDDu, 42, orig, sizeof orig, buf, sizeof buf);
...
START_TEST(rtx_build_increments_seq_independent_of_orig_seq) {
  ...
  rtx_build(&seq, 1, 99, 0, 5, NULL, 0, b1, sizeof b1);
  rtx_build(&seq, 1, 99, 0, 5, NULL, 0, b2, sizeof b2); /* same orig_seq both times */
  rtx_parse(b1, 14, 99, &o1);
  rtx_parse(b2, 14, 99, &o2);

  ck_assert_uint_eq(o1.osn, 5u);
  ck_assert_uint_eq(o2.osn, 5u);
  /* RTX stream's own RTP seq (not exposed by rtx_pkt_t) must still
     differ between calls; check indirectly via raw bytes */
...
START_TEST(rtx_build_rejects_small_cap) {
  ...
  ck_assert_uint_eq(rtx_build(&seq, 1, 99, 0, 0, NULL, 0, buf, sizeof buf), 0u);
}
END_TEST
...
static _Atomic uint16_t g_rtx_race_seq;
...
static void *rtx_race_worker(void *arg) {
  ...
  for (i = 0; i < RTX_RACE_PER_THREAD; i++) {
    ...
    rtx_build(&g_rtx_race_seq, 0x99u, 96, 0, 0, NULL, 0, buf, sizeof buf);
