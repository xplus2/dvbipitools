...
#include "dipidescramble/secasm.h"
...
START_TEST(single_packet_section_completes_immediately) {
  ...
  ck_assert_int_eq(secasm_feed(&a, pl, 1 + seclen, 1), 1);
  out = secasm_section(&a, &outlen);
  ck_assert_uint_eq(outlen, seclen);
  ck_assert_mem_eq(out, sec, seclen);
}
END_TEST
...
START_TEST(multi_packet_section_reassembles) {
  ...
  ck_assert_int_eq(secasm_feed(&a, pl1, sizeof pl1, 1), 0); /* not complete yet */

  rest = seclen - first_chunk;
  ck_assert_uint_le(rest, sizeof pl2);
  memcpy(pl2, sec + first_chunk, rest);
  ck_assert_int_eq(secasm_feed(&a, pl2, rest, 0), 1);
...
START_TEST(pointer_field_skips_leftover_stuffing) {
  ...
  ck_assert_int_eq(secasm_feed(&a, pl, 1 + 4 + seclen, 1), 1);
  out = secasm_section(&a, &outlen);
  ck_assert_uint_eq(outlen, seclen);
  ck_assert_mem_eq(out, sec, seclen);
}
...
START_TEST(feed_without_prior_pusi_is_ignored) {
  ...
  memset(&a, 0, sizeof a);
  ck_assert_int_eq(secasm_feed(&a, pl, sizeof pl, 0), 0);
}
END_TEST
...
START_TEST(oversized_section_length_resets_without_crash) {
  ...
  pl[0] = 0x00;
  pl[1] = 0x82;
  pl[2] = 0x7F; /* section_length top nibble 0x7 -> 0xFFF, far larger than SECASM_BUF_LEN */
  pl[3] = 0xFF;

  ck_assert_int_eq(secasm_feed(&a, pl, sizeof pl, 1), 0);
  /* recovers cleanly on the next section */
  {
    ...
    ck_assert_int_eq(secasm_feed(&a, pl2, 1 + seclen, 1), 1);
    out = secasm_section(&a, &outlen);
    ck_assert_uint_eq(outlen, seclen);
    ck_assert_mem_eq(out, sec, seclen);
  }
...
START_TEST(second_section_after_first_resets_state) {
  ...
  ck_assert_int_eq(secasm_feed(&a, pl, 1 + len_a, 1), 1);
  out = secasm_section(&a, &outlen);
  ck_assert_uint_eq(outlen, len_a);
  ck_assert_mem_eq(out, sec_a, len_a);

  pl[0] = 0x00;
  memcpy(pl + 1, sec_b, len_b);
  ck_assert_int_eq(secasm_feed(&a, pl, 1 + len_b, 1), 1);
  out = secasm_section(&a, &outlen);
  ck_assert_uint_eq(outlen, len_b);
  ck_assert_mem_eq(out, sec_b, len_b);
}
END_TEST

static Suite *secasm_suite(void) {
  Suite *s = suite_create("secasm");
...
int main(void) {
  SRunner *sr = srunner_create(secasm_suite());
