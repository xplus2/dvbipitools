...
static void *reader_main(void *arg) {
  ...
  while (!atomic_load_explicit(&r->io.stop, memory_order_relaxed) && !signal_stop_requested()) {
    ...
    if (n < 0) break;
    if (n == 0) continue;
    if (pipe_write_all(r->io.pfd[1], buf, (size_t)n, &r->io.stop) < 0) break;
  }
  srtin_close(s);
  close(r->io.pfd[1]); /* next read() on pfd[0] sees EOF, stop or error alike */
  return NULL;
}
...
srtsrc_t *srtsrc_open(const srtsrc_cfg_t *cfg) {
  ...
  if (cfg->passphrase) bufcpy(r->passphrase, sizeof r->passphrase, cfg->passphrase);
  if (cfg->streamid) bufcpy(r->streamid, sizeof r->streamid, cfg->streamid);
  if (cfg->packetfilter) bufcpy(r->packetfilter, sizeof r->packetfilter, cfg->packetfilter);
...
int srtsrc_fd(const srtsrc_t *r) { return pipereader_fd(&r->io); }
