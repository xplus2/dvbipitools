...
http_t *http_get(const http_url_t *url, const char *user_agent, int insecure, const char *extra_header);
...
/* HTTP status of the (possibly redirected-to) response. Includes 304 Not Modified
   ...
   http_get() returns that case too (body empty) instead of treating it as failure */
