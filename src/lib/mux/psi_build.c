...
static size_t psi_put_sdt_header(unsigned char *out, unsigned version, unsigned tsid, unsigned onid) {
  size_t n = 0;
  out[n++] = 0x42;
  n += 2;
  psi_put16(out + n, tsid);
...
size_t psi_build_sdt(unsigned version, unsigned tsid, unsigned onid, unsigned service_id, unsigned service_type, const char *provider, const char *service, unsigned char *out, size_t cap) {
  psi_sdt_entry_t e = {service_id, service_type, provider, service};
  size_t n;

  if (cap < 24) return 0;
  n = psi_put_sdt_header(out, version, tsid, onid);
  n = psi_put_sdt_entry(out, n, cap, &e);
  if (!n) return 0;
