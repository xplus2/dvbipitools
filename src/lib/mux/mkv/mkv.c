...
mkv_t *mkv_new(int fd, const mkv_opts_t *opts, int video_ok, unsigned long long *bytes, const unsigned *pmt_pids, int n_pids) {
  ...
  m->video_ok = video_ok;
  m->bytes = bytes;
  if (n_pids > MKV_MAX_PROGRAMS) n_pids = MKV_MAX_PROGRAMS;
...
void mkv_feed(mkv_t *m, const unsigned char *pkt) {
  ...
  if (!m->setup && all_psi_ready(m)) setup(m);
  if (m->setup) pes_feed(m->pes, pkt);
  if (m->setup && !m->started && m->ntrk) all_ready(m); /* re-check: SDT may arrive later */
}
