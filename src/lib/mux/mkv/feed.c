...
static void try_parse_vvc_hdr(mkv_t *m, track_t *t) {
...
static void try_parse_av1_hdr(mkv_t *m, track_t *t) {
  av1_seq_hdr_t info;
...
static void handle_video(mkv_t *m, track_t *t, int has_pts, uint64_t pts, const unsigned char *d, size_t len) {
  ...
  if (key) t->got_key = 1;
  if (t->hdr_parsed && t->got_key && t->vbuflen) emit(m, t, t->vbuf, t->vbuflen, key);
}
...
static void handle_mpeg2(mkv_t *m, track_t *t, int has_pts, uint64_t pts, const unsigned char *d, size_t len) {
  ...
  if (key) t->got_key = 1;
  if (t->hdr_parsed && t->got_key) emit(m, t, d, len, key);
}
...
static void handle_audio(mkv_t *m, track_t *t, int has_pts, uint64_t pts, const unsigned char *data, size_t len) {
  ...
  while (pos < t->remlen) {
    ...
    if (f.outlen) emit(m, t, f.out, f.outlen, t->es.codec == CODEC_AC4 ? f.ac4_iframe : 1);
...
static void add_track(mkv_t *m, const psi_es_t *es, int psi_idx) {
  ...
  if (pes_track(m->pes, t->pid)) log_line("mkv: pes tracker full, pid 0x%04x not reassembled", t->pid);
  m->ntrk++;
}
...
void setup(mkv_t *m) {
  ...
  if (m->video_ok) {
    ...
    for (k = 0; k < c && m->ntrk < MKV_MAX_TRACKS; k++) {
      ...
      if (es[k].codec != CODEC_H264 && es[k].codec != CODEC_HEVC && es[k].codec != CODEC_VVC && es[k].codec != CODEC_AV1 && es[k].codec != CODEC_MPEG2V) {
