...
flv_t *flv_new(const flv_opts_t *opts, unsigned pmt_pid, flv_tag_cb cb, void *cb_ctx, unsigned long long *bytes) {
  ...
  if (pmt_pid) psi_select_pmt_pid(f->psi, pmt_pid);
...
void flv_feed(flv_t *f, const unsigned char *pkt) {
  ...
  if (f->err) return;
  pid = tspack_pid(pkt);
  if (psi_wants_pid(f->psi, pid)) psi_feed(f->psi, pkt);
  if (!f->setup && psi_ready(f->psi)) flv_setup(f);
  if (f->setup) pes_feed(f->pes, pkt);
}
