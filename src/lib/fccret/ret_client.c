...
static void gap_shift(ret_client_t *r) {
  for (size_t i = 0; i + 1 < r->hold_n; i++) r->hold[i] = r->hold[i + 1];
  if (r->hold_n) r->hold_n--;
  r->expected_seq++;
}
  ...
  for (;;) {
...

    {
      ...
      if ((size_t)delta < r->hold_n) {
        if (!r->hold[delta].used) hold_store(r, (size_t)delta, payload, len);
        return;
      }
...
void on_repair(ret_client_t *r, const unsigned char *pkt, size_t len, double now) {
  ...
  if (delta < 0 || (size_t)delta >= r->hold_n || r->hold[delta].used) return;
  r->oversized_payload_logged = 0;
  r->hold[delta].used = 1;
...
ssize_t ret_client_read(ret_client_t *r, mcast_t *main, unsigned char *buf, size_t cap) {
  ...
  if (pfd[ui].revents & POLLIN) {
    ...
    if (rn > 0) on_repair(r, raw, (size_t)rn, now);
  }
