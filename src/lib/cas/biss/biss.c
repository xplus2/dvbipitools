...
static int aes128_ecb_block(const unsigned char key[BISS_KEY_LEN], const unsigned char in[BISS_KEY_LEN], unsigned char out[BISS_KEY_LEN], int enc) {
  ...
  if (enc ? EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, key, NULL) != 1
          : EVP_DecryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, key, NULL) != 1)
    goto out;
  EVP_CIPHER_CTX_set_padding(ctx, 0);

  if (enc ? EVP_EncryptUpdate(ctx, out, &outlen, in, BISS_KEY_LEN) != 1
          : EVP_DecryptUpdate(ctx, out, &outlen, in, BISS_KEY_LEN) != 1)
    goto out;
  if (outlen != BISS_KEY_LEN)
    goto out;
  totlen = outlen;
