...
void metrics_exporter_init(metrics_exporter_t *exp, metrics_component_t component, const char *metrics_id, const char *sock_path, double interval_s) {
  ...
  exp->fd = -1;
  if (!metrics_id || !metrics_id[0]) return;
  if (!sock_path || !sock_path[0]) sock_path = METRICS_DEFAULT_SOCK_PATH;
  if (interval_s <= 0.0) interval_s = METRICS_DEFAULT_INTERVAL_S;
  if (strlen(sock_path) >= sizeof exp->dest.sun_path) return;
...
int metrics_exporter_due(metrics_exporter_t *exp, double now_mono) {
  ...
  if (exp->last_send_mono != 0.0 && now_mono - exp->last_send_mono < exp->interval_s) return 0;
  exp->last_send_mono = now_mono;
  return 1;
}

void metrics_exporter_note_error(metrics_exporter_t *exp, net_err_reason_t reason) {
  if (!exp->enabled || (unsigned)reason >= NET_ERR_COUNT) return;
...
void metrics_push_entries(metrics_exporter_t *exp, const char *version, const metrics_entry_t *entries, size_t n) {
  metrics_writer_t w;
  if (metrics_exporter_begin(exp, &w, version)) return;
  for (size_t i = 0; i < n; i++) metrics_writer_put(&w, entries[i].id, entries[i].label, entries[i].value);
