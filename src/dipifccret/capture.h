...
typedef void (*capture_frame_cb)(int family, const void *addr, size_t addr_len, unsigned port, uint32_t ssrc, uint16_t seq, uint32_t timestamp, const unsigned char *payload, size_t payload_len, void *user);
...
/* builds the classic-BPF program matching ethertype+optional single vlan+dst-net ranges; caller frees; NULL on error (too many ranges, or out of memory); exposed for testing */
