...
static int cw_gen(unsigned char *out, size_t len) {
  ...
  while (got < len) {
    ...
    if (n < 0) {
      if (errno == EINTR)
        continue;
      log_line(TOOL_NAME ": ecmg: getrandom: %s", strerror(errno));
      return -1;
    }
    got += (size_t)n;
  }
...
int ecmg_cw_valid_calc(ecmg_resilience_t resilience, int connected, unsigned long packets_per_cp, unsigned long cur, unsigned long published_at) {
  if (resilience != ECMG_RESILIENCE_UNSCRAMBLED)
    return 1;
  if (connected)
    return 1;
  if (packets_per_cp == 0)
    return 1;
  return (cur - published_at) < packets_per_cp;
}
...
int ecmg_target_parity_calc(ecmg_resilience_t resilience, int connected, unsigned long packets_per_cp, unsigned long cur, unsigned long published_at, unsigned long epoch) {
  ...
  if (resilience != ECMG_RESILIENCE_CYCLING)
...
/* --cas-resilience=unscrambled: cached CW expires one CP after the outage started (fails to
   clear on that pid rather than run a key past its intended validity window). frozen/cycling
   never expire it - they keep using known CW material, just differ in which slot (see
   ecmg_client_target_parity()). */
int ecmg_client_cw_valid(ecmg_client_t *c) {
  ...
  return ecmg_cw_valid_calc(c->cfg.resilience, connected, c->packets_per_cp, cur, published_at);
}
...
int ecmg_client_target_parity(ecmg_client_t *c) {
  ...
  unsigned long published_at = atomic_load_explicit(&c->cw_published_at, memory_order_relaxed);
  int connected = atomic_load_explicit(&c->connected, memory_order_relaxed);
  return ecmg_target_parity_calc(c->cfg.resilience, connected, c->packets_per_cp, cur, published_at, epoch);
}
...
static int tcp_dial(ecmg_client_t *c, const char *host, unsigned port) {
  ...
  if (e) {
    log_line(TOOL_NAME ": ecmg: resolve %s: %s", host, gai_strerror(e));
    return -1;
  }
  ...
  if (fd < 0) {
    if (!ecmg_stopping(c))
      log_line(TOOL_NAME ": ecmg: connect %s:%u: %s", host, port, strerror(save_errno));
    return -1;
  }
...
int ecmg_parse_channel_status(const unsigned char *body, size_t body_len, unsigned *out_lead_cw, unsigned *out_cw_per_msg, unsigned *out_max_comp_time_ms, unsigned *out_min_cp_100ms, unsigned *out_ecm_rep_period_ms) {
  ...
  simulcrypt_tlv_reader_init(&it, body, body_len);
  while ((rc = simulcrypt_tlv_reader_next(&it, &tag, &val, &vlen)) == 1) {
    if (tag == ECMG_P_LEAD_CW && vlen == 1)
      lead_cw = val[0];
    else if (tag == ECMG_P_CW_PER_MSG && vlen == 1)
      cw_per_msg = val[0];
    else if (tag == ECMG_P_MAX_COMP_TIME && vlen == 2)
      max_comp_time_ms = ((unsigned)val[0] << 8) | val[1];
    else if (tag == ECMG_P_MIN_CP_DURATION && vlen == 2)
      min_cp_100ms = ((unsigned)val[0] << 8) | val[1];
    else if (tag == ECMG_P_ECM_REP_PERIOD && vlen == 2)
...
static int connect_and_setup(ecmg_client_t *c, int *out_fd, unsigned char *out_version, unsigned *out_lead_cw, unsigned *out_cw_per_msg, unsigned *out_max_comp_time_ms) {
  ...
  for (;;) {
    ...
    if (wait_for_message(c, &rd, fd, ECMG_HANDSHAKE_TIMEOUT_MS, &hdr, &payload) != 1) {
      if (!ecmg_stopping(c))
        log_line(TOOL_NAME ": ecmg: no channel_status/channel_error reply");
      close(fd);
      return -1;
    }
    if (hdr.type == ECMG_MSG_CHANNEL_ERROR) {
      ...
      log_line(TOOL_NAME ": ecmg: channel_setup rejected, error_status=0x%04x", err);
      return -1;
    }
    if (hdr.type != ECMG_MSG_CHANNEL_STATUS) {
      log_line(TOOL_NAME ": ecmg: unexpected reply 0x%04x to channel_setup", hdr.type);
      close(fd);
      return -1;
    }

    {
      ...
      if (ecmg_parse_channel_status(payload, hdr.payload_len, &lead_cw, &cw_per_msg, &max_comp_time_ms, &min_cp_100ms, &ecm_rep_period_ms) < 0) {
        log_line(TOOL_NAME ": ecmg: malformed or unusable channel_status");
        close(fd);
        return -1;
      }
      if (min_cp_100ms && c->cfg.cp_duration_ms < min_cp_100ms * 100)
        log_line(TOOL_NAME ": ecmg: --cas-cp-duration %ums is below the ECMG's min_CP_duration %ums, using it anyway", c->cfg.cp_duration_ms, min_cp_100ms * 100);
      *out_lead_cw = lead_cw;
      *out_cw_per_msg = cw_per_msg;
    ...
    if (wait_for_message(c, &rd, fd, ECMG_HANDSHAKE_TIMEOUT_MS, &hdr, &payload) != 1) {
      if (!ecmg_stopping(c))
        log_line(TOOL_NAME ": ecmg: no stream_status/stream_error reply");
      close(fd);
      return -1;
    }
    if (hdr.type != ECMG_MSG_STREAM_STATUS) {
      ...
      if (hdr.type == ECMG_MSG_STREAM_ERROR)
        ecmg_find_error_status(payload, hdr.payload_len, &err);
      log_line(TOOL_NAME ": ecmg: stream_setup rejected, reply=0x%04x error_status=0x%04x", hdr.type, err);
      close(fd);
      return -1;
    }
    ...
    *out_fd = fd;
    *out_version = version;
    log_line(TOOL_NAME ": ecmg: channel+stream established (protocol_version=0x%02x, lead_cw=%u, cw_per_msg=%u)", version, *out_lead_cw, *out_cw_per_msg);
    return 0;
  }
}
...
static int run_steady_state(ecmg_client_t *c, int fd, unsigned char version, unsigned lead_cw, unsigned cw_per_msg, unsigned max_comp_time_ms) {
  ...
  while (!ecmg_stopping(c)) {
    ...
    if (c->packets_per_cp == 0 || cur + c->lookahead_margin_packets >= next_boundary) {
      ...
      if (!len) {
        log_line(TOOL_NAME ": ecmg: failed to build CW_provision");
        return -1;
      }
      ...
      if (simulcrypt_send_all(fd, msg, len, ECMG_HANDSHAKE_TIMEOUT_MS) < 0) {
        log_line(TOOL_NAME ": ecmg: CW_provision send failed");
        return -1;
      }
      if (wait_for_message(c, &rd, fd, (int)max_comp_time_ms + ECMG_HANDSHAKE_TIMEOUT_MS, &hdr, &payload) != 1) {
        if (!ecmg_stopping(c))
          log_line(TOOL_NAME ": ecmg: no ECM_response for CP %u", cp_number);
        return -1;
      }
      if (hdr.type == ECMG_MSG_ECM_RESPONSE) {
        ...
        unsigned short err = 0;
        ecmg_find_error_status(payload, hdr.payload_len, &err);
        log_line(TOOL_NAME ": ecmg: CW_provision rejected, reply=0x%04x error_status=0x%04x", hdr.type, err);
        return -1;
      }
      ...
      if (rc < 0) {
        log_line(TOOL_NAME ": ecmg: connection lost");
        return -1;
      }
      if (rc == 1 && (hdr.type == ECMG_MSG_CHANNEL_ERROR || hdr.type == ECMG_MSG_STREAM_ERROR)) {
        unsigned short err = 0;
        ecmg_find_error_status(payload, hdr.payload_len, &err);
        log_line(TOOL_NAME ": ecmg: async error 0x%04x, error_status=0x%04x", hdr.type, err);
        return -1;
      }
    }
  }
...
ecmg_client_t *ecmg_client_start(const ecmg_client_cfg_t *cfg, const atomic_ulong *packet_counter, unsigned long packets_per_cp, unsigned long lookahead_margin_packets) {
  ...
  if (pthread_create(&c->thread, NULL, ecmg_client_main, c) != 0) {
    log_line(TOOL_NAME ": ecmg: pthread_create: %s", strerror(errno));
