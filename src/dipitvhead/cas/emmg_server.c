...
#define EMMG_MAX_CONNS 4
#define EMMG_QUEUE_CAP 64
...
static void publish_datagram_cb(const unsigned char *data, unsigned short len, void *user) {
  ...
  if (len > EMMG_MAX_DATAGRAM_LEN) {
    log_line(TOOL_NAME ": emmg: dropping oversized EMM datagram (%u bytes)", (unsigned)len);
    return;
  }
  pthread_mutex_lock(&s->queue_lock);
  if (s->queue_len == EMMG_QUEUE_CAP) {
    s->queue_head = (s->queue_head + 1) % EMMG_QUEUE_CAP;
    s->queue_len--;
    log_line(TOOL_NAME ": emmg: EMM queue full, dropping oldest datagram");
  }
  {
    size_t idx = (s->queue_head + s->queue_len) % EMMG_QUEUE_CAP;
...
static void handle_message(emmg_server_t *s, emmg_conn_state_t *cs, unsigned char version, unsigned short type,
  ...
  *should_close = 0;
  ...
  log_line(TOOL_NAME ": emmg: rx version=0x%02x type=0x%04x body_len=%zu", version, type, body_len);
  ...
  case EMMG_MSG_STREAM_ERROR: {
    ...
    log_line(TOOL_NAME ": emmg: client reported error (message 0x%04x, error_status 0x%04x)", type, err);
    *should_close = 1;
    break;
  }
  case EMMG_MSG_DATA_PROVISION: {
    ...
    log_line(TOOL_NAME ": emmg: data_provision, %d datagram(s) queued", n);
    break;
  }
  default:
...
static void *worker_main(void *arg) {
  ...
  log_line(TOOL_NAME ": emmg: connection accepted (slot %d)", slot);
  ...
  while (!atomic_load_explicit(&s->stop, memory_order_relaxed) && !signal_stop_requested()) {
    ...
    if (rc < 0) {
      log_line(TOOL_NAME ": emmg: connection closed by peer or read error (slot %d)", slot);
      break;
    }
    if (rc == 0)
      continue;
    ...
    if (reply_len) {
      log_line(TOOL_NAME ": emmg: tx version=0x%02x type=0x%04x", reply[0], ((unsigned)reply[1] << 8) | reply[2]);
      if (simulcrypt_send_all(fd, reply, reply_len, EMMG_SEND_TIMEOUT_MS) < 0) {
        log_line(TOOL_NAME ": emmg: reply send failed (slot %d)", slot);
        break;
      }
    }
    if (should_close)
...
static int tcp_listen_dualstack(unsigned port) {
  ...
  if (fd < 0) {
    log_line(TOOL_NAME ": emmg: socket: %s", strerror(errno));
    return -1;
  }
  ...
  if (bind(fd, (struct sockaddr *)&addr, sizeof addr) < 0) {
    log_line(TOOL_NAME ": emmg: bind :%u: %s", port, strerror(errno));
    close(fd);
    return -1;
  }
  if (listen(fd, 8) < 0) {
    log_line(TOOL_NAME ": emmg: listen: %s", strerror(errno));
    close(fd);
    return -1;
  }
  flags = fcntl(fd, F_GETFL, 0);
  ...
  while (!atomic_load_explicit(&s->stop, memory_order_relaxed) && !signal_stop_requested()) {
    ...
    if (fd < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR)
        continue;
      log_line(TOOL_NAME ": emmg: accept: %s", strerror(errno));
      continue;
    }
    ...
    if (slot < 0) {
      log_line(TOOL_NAME ": emmg: connection limit (%d) reached, rejecting", EMMG_MAX_CONNS);
      close(fd);
      continue;
    }

    {
      ...
      if (pthread_create(&th, NULL, worker_main, wa) != 0) {
        log_line(TOOL_NAME ": emmg: pthread_create: %s", strerror(errno));
        close(fd);
        free(wa);
        atomic_store_explicit(&s->worker_active[slot], 0, memory_order_release);
      }
