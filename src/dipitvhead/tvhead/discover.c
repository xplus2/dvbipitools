...
static int psi_cb_inspect(void *v, const unsigned char *pkt) {
  discover_feed_t *df = v;
  tsinspect_packet(df->insp, pkt);
  psi_feed(df->psi, pkt);
  return 0;
}
...
void print_discovered(const psi_t *psi) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (e->lang[0]) log_line("  pid 0x%x: %s (%s) lang=%s", e->pid, pid_class_name(e->cls), codec_name(e->codec), e->lang);
    else            log_line("  pid 0x%x: %s (%s)", e->pid, pid_class_name(e->cls), codec_name(e->codec));
  }
  if (*psi_service_name(psi)) log_line("  SDT: service=\"%s\" provider=\"%s\"", psi_service_name(psi), psi_provider_name(psi));
  if (*psi_network_name(psi)) log_line("  NIT: network=\"%s\"", psi_network_name(psi));
}
...
int discover_insp(tvsrc_t *src, const dipitvhead_input_t *input, psi_t *psi, input_metrics_t *im, tsinspect_t *insp) {
  ...
  while (!signal_stop_requested()) {
    ...
    if (r) return r;
    if (mono_seconds() - start >= DISCOVERY_TIMEOUT_S) return 0;
  }
  return -1;
}

int discover(tvsrc_t *src, const dipitvhead_input_t *input, psi_t *psi, input_metrics_t *im) {
  return discover_insp(src, input, psi, im, NULL);
}
