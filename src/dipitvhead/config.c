...
static int set_table(table_mode_t *mode, char *text, size_t sz, const char *v, char *e, size_t n) {
  if (!strcmp(v, "-")) {
    *mode = TABLE_DROP;
    return 0;
  }
  if (set_buf(text, sz, v, e, n)) return -1;
  *mode = TABLE_OVERRIDE;
  return 0;
}
...
static dipitvhead_input_t *cur_input(config_t *cfg, char *e, size_t n) {
  if (!item.input) {
...
static int apply_input_pmt_pid(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  if (!in) return -1;
...
static int apply_input_sdt(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? set_table(&in->sdt_mode, in->sdt_text, sizeof in->sdt_text, v, e, n) : -1;
}
...
static int apply_input_iface(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? yamlcfg_set_str(&in->iface_in, v, e, n) : -1;
}

static int apply_input_strip_eit(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? yamlcfg_set_bool(&in->strip_eit, v, e, n) : -1;
}

static int apply_input_strip(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  if (!in) return -1;
...
static int apply_input_hbbtv(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? yamlcfg_set_str(&in->hbbtv_url, v, e, n) : -1;
}

static int apply_input_hbbtv_org_id(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? yamlcfg_set_uint(&in->hbbtv_org_id, v, 1, UINT_MAX, e, n) : -1;
}

static int apply_input_hbbtv_app_id(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? yamlcfg_set_uint(&in->hbbtv_app_id, v, 1, 0xFFFF, e, n) : -1;
}

static int apply_input_rist_profile(void *c, const char *v, char *e, size_t n) {
...
static int apply_input_srt_passphrase(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? set_buf(in->srt_passphrase_in, sizeof in->srt_passphrase_in, v, e, n) : -1;
}

static int apply_input_srt_pbkeylen(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? set_pbkeylen(&in->srt_pbkeylen_in, v, e, n) : -1;
}

static int apply_input_srt_streamid(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? set_buf(in->srt_streamid_in, sizeof in->srt_streamid_in, v, e, n) : -1;
}

static int apply_input_srt_packetfilter(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
  return in ? set_buf(in->srt_packetfilter_in, sizeof in->srt_packetfilter_in, v, e, n) : -1;
}

static int apply_input_srt_latency(void *c, const char *v, char *e, size_t n) {
  dipitvhead_input_t *in = cur_input(c, e, n);
...
static int apply_udp(void *c, const char *v, char *e, size_t n) {
  int udp;
...
static int apply_nit(void *c, const char *v, char *e, size_t n) {
  config_t *cfg = c;
  return set_table(&cfg->nit_mode, cfg->nit_text, sizeof cfg->nit_text, v, e, n);
}
...
static int apply_bitrate(void *c, const char *v, char *e, size_t n) {
  return yamlcfg_set_uint(&((config_t *)c)->bitrate_kbps, v, 1, 1000000, e, n);
}

static int apply_stuff(void *c, const char *v, char *e, size_t n) {
  return yamlcfg_set_bool(&((config_t *)c)->stuff, v, e, n);
}

static int apply_burst_limit(void *c, const char *v, char *e, size_t n) {
...
static int apply_cas_pids(void *c, const char *v, char *e, size_t n) {
  config_t *cfg = c;
  cfg->any_cas_flag = 1;
...
{"input.pmt-pid", apply_input_pmt_pid, 0, 0},
{"input.sid", apply_input_sid, 0, 0},
{"input.sdt", apply_input_sdt, 0, 0},
{"input.provider", apply_input_provider, 0, 0},
{"input.iface", apply_input_iface, 0, 0},
{"input.strip-eit", apply_input_strip_eit, 0, 0},
{"input.strip", apply_input_strip, 0, 0},
{"input.hbbtv", apply_input_hbbtv, 0, 0},
{"input.hbbtv-org-id", apply_input_hbbtv_org_id, 0, 0},
{"input.hbbtv-app-id", apply_input_hbbtv_app_id, 0, 0},
{"input.rist-profile-in", apply_input_rist_profile, 0, 0},
{"input.srt-passphrase-in", apply_input_srt_passphrase, 0, 0},
{"input.srt-pbkeylen-in", apply_input_srt_pbkeylen, 0, 0},
{"input.srt-streamid-in", apply_input_srt_streamid, 0, 0},
{"input.srt-packetfilter-in", apply_input_srt_packetfilter, 0, 0},
{"input.srt-latency-in", apply_input_srt_latency, 0, 0},
...
{"bitrate", apply_bitrate, 0, 0},
{"stuff", apply_stuff, 0, 0},
{"burst-limit", apply_burst_limit, 0, 0},
...
int tvh_cfg_load(config_t *cfg, const char *path) {
  yamlcfg_t y;
  return yamlcfg_load_items(&y, TOOL_NAME, 0, path, DEFAULT_CONFIG_PATH, keys, sizeof keys / sizeof *keys, cfg, item_hook) == YAMLCFG_ERROR ? -1 : 0;
}
...
int tvh_cfg_test(const char *path) {
  ...
  if (yamlcfg_load_items(&y, TOOL_NAME, 1, path, DEFAULT_CONFIG_PATH, keys, sizeof keys / sizeof *keys, &cfg, item_hook) != YAMLCFG_LOADED) return -1;
