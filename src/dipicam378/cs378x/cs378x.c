...
cs378x_server_t *cs378x_server_start(const cs378x_cfg_t *cfg, cs378x_ecm_cb ecm_cb, cs378x_emm_cb emm_cb, void *user) {
  ...
  s = calloc(1, sizeof *s);
  if (!s) return NULL;
  for (int i = 0; i < CS378X_MAX_CONNS; i++) atomic_init(&s->worker_fd[i], -1);
...
const char *cs378x_auth_reason_name(cam_auth_reason_t r) {
  switch (r) {
    case CAM_AUTH_USER:      return "user";
    case CAM_AUTH_CONNID:    return "connid";
    case CAM_AUTH_CHECKSUM:  return "checksum";
    case CAM_AUTH_OVERSIZED: return "oversized";
...
void cs378x_server_get_metrics(const cs378x_server_t *s, cs378x_metrics_t *out) {
  ...
  for (int i = 0; i < CS378X_MAX_CONNS; i++) out->connections_active += (unsigned)atomic_load_explicit(&s->worker_active[i], memory_order_relaxed);
  out->connections_total = atomic_load_explicit(&s->connections_total, memory_order_relaxed);
  for (int i = 0; i < CAM_AUTH_REASON_COUNT; i++) out->auth_errors_total[i] = atomic_load_explicit(&s->auth_errors_total[i], memory_order_relaxed);
