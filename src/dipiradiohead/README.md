...
All options (except `-h`, `-c`, `--config-strict` and `--configtest`) can be set in a YAML file, see [dipiradiohead.yaml](dipiradiohead.yaml).
...
* `-R srt://` output needs libsrt (`DVBIPITOOLS_SRT` / `HAVE_SRT`, toolkit-wide).
...
See [dipiradiohead_mux01.service](dipiradiohead_mux01.service) for a reasonable starting point.
