...
static h2_body_src_t *h2_body_src_alloc(void) {
  if (t_h2_body_src_pool_n > 0) return t_h2_body_src_pool[--t_h2_body_src_pool_n];
...
static ssize_t body_read_cb(nghttp2_session *ng, int32_t stream_id, uint8_t *buf, size_t length, uint32_t *data_flags, nghttp2_data_source *source, void *ud) {
  ...
  if (n) memcpy(buf, src->data + src->off, n);
  src->off += n;
  if (src->off >= src->len) {
...
void h2_submit_resp(h2_conn_t *conn, int32_t stream_id, int status, const char *content_type, const char *etag, size_t content_length, uint8_t *body, int zc, const char *origin_hdr) {
  ...
  if (content_type)
    nva[nvlen++] = (nghttp2_nv){(uint8_t *)"content-type", (uint8_t *)content_type, 12, strlen(content_type), NGHTTP2_NV_FLAG_NONE};
...
int h2_llhls_try_park(h2_conn_t *conn, const conn_t *c, int32_t stream_id, capture_ctx_t *ctx, const pid_filter_t *filter, unsigned pmt_pid, const char *filename,
  ...
  return llhls_waiter_pool_try_park(t_h2_llhls_waiters, H2_LLHLS_WAITERS_MAX, &t_h2_llhls_waiters_active, conn, stream_id, ctx, filter, pmt_pid, filename,
...
static void h2_llhls_finish(llhls_waiter_t *w) {
  ...
  if (!hls_render_ll(w->cap_ctx, &w->filter, w->pmt_pid, w->filename, w->is_head, w->inm[0] ? w->inm : NULL, &resp)) {
    ...
    h2_submit_resp(conn, (int32_t)w->stream_id, resp.status, resp.content_type, resp.etag, resp.body_len, resp.body, resp.zc, w->origin[0] ? w->origin : NULL);
...
void h2_llhls_flush_waiters(void) {
  llhls_waiter_pool_flush(t_h2_llhls_waiters, H2_LLHLS_WAITERS_MAX, &t_h2_llhls_waiters_active, llhls_ready_part, h2_llhls_finish);
}
...
int h2_hls_cold_try_park(h2_conn_t *conn, int32_t stream_id, capture_ctx_t *ctx, const pid_filter_t *filter, unsigned pmt_pid, const char *filename, hls_cold_kind_t kind, seg_container_t container,
  ...
  return hls_cold_waiter_pool_try_park(t_h2_hls_cold_waiters, H2_HLS_COLD_WAITERS_MAX, &t_h2_hls_cold_waiters_active, conn, stream_id, ctx, filter, pmt_pid, filename,
...
void h2_hls_cold_flush_waiters(void) {
  llhls_waiter_pool_flush(t_h2_hls_cold_waiters, H2_HLS_COLD_WAITERS_MAX, &t_h2_hls_cold_waiters_active, hls_cold_ready, h2_hls_cold_finish);
}
