...
static void buf_compact(conn_buf *b) {
  ...
  if (rem) memmove(b->buf, b->buf + b->off, rem);
  b->len = rem;
  b->off = 0;
}
...
static _Thread_local conn_retiree_t t_conn_retiring[CONN_POOL_MAX];
static _Thread_local int t_conn_retiring_n;

static void conn_pool_reclaim(void) {
  for (int i = 0; i < t_conn_retiring_n;) {
    if (qsbr_mark_passed(reactor_qsbr(), t_conn_retiring[i].mark)) {
      conn_t *c = t_conn_retiring[i].c;
      t_conn_retiring[i] = t_conn_retiring[--t_conn_retiring_n];
      free(c->in.buf);
      free(c->out.buf);
...
void conn_free(conn_t *c) {
  ...
  conn_pool_reclaim();
  if (t_conn_retiring_n < CONN_POOL_MAX) {
    qsbr_mark(reactor_qsbr(), t_conn_retiring[t_conn_retiring_n].mark);
    t_conn_retiring[t_conn_retiring_n].c = c;
    t_conn_retiring_n++;
  } else {
    log_line(TOOL_NAME ": conn retire queue full, leaking one conn_t");
  }
}
...
int conn_table_capacity(int max_clients) {
  ...
  if (n < CONN_TABLE_FLOOR) n = CONN_TABLE_FLOOR;
  if (n > CONN_TABLE_MAX) n = CONN_TABLE_MAX;
  return n;
}

int conn_table_init(int maxfd) {
  if (g_fd_conn_max || maxfd <= 0) return 0;
  if (maxfd > CONN_TABLE_MAX) maxfd = CONN_TABLE_MAX;
...
void conn_publish(conn_t *c) {
  if (!g_fd_conn_max || c->fd < 0) return;
...
conn_t *conn_for_fd(int fd) {
  if (!g_fd_conn_max || fd < 0) return NULL;
  if (fd >= g_fd_conn_max) {
...
int conn_send_buffered3(conn_t *c, const void *a, size_t alen, const void *b, size_t blen, const void *cc, size_t clen) {
  ...
  if (pending + alen + blen + clen > CONN_OUT_MAX) {
    c->dead = 1; /* slow consumer: owner closes on next visit */
    rc = -1;
  } else if ((alen && conn_queue(c, a, alen) < 0) || (blen && conn_queue(c, b, blen) < 0) || (clen && conn_queue(c, cc, clen) < 0)) {
    c->dead = 1; /* OOM */
    rc = -1;
  }
...
int conn_send_buffered(conn_t *c, const void *a, size_t alen, const void *b, size_t blen) {
  return conn_send_buffered3(c, a, alen, b, blen, NULL, 0);
}
...
void conn_zc_drain(conn_t *c) {
  int hi = -1;
  if (!tls_zc_drain(c->fd, &hi)) return;
  if (hi > c->zc_confirmed_id) c->zc_confirmed_id = hi;
  int w = 0;
  for (int i = 0; i < c->zc_await_count; i++) {
    if (c->zc_await[i].zc_id_hi <= c->zc_confirmed_id) c->zc_await[i].release(c->zc_await[i].release_arg);
    else c->zc_await[w++] = c->zc_await[i];
  }
  c->zc_await_count = w;
}
