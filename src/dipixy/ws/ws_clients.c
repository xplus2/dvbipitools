...
ws_stripe_t g_stripes[WS_STRIPE_COUNT_MAX];
int g_stripe_count;
...
static void hash_insert(ws_stripe_t *stripe, uint32_t h, int idx) {
  uint32_t i = h & stripe->hash_mask;
  for (uint32_t n = 0; n < stripe->hash_cap; n++, i = (i + 1) & stripe->hash_mask) {
    if (stripe->hash[i] == WS_HASH_EMPTY || stripe->hash[i] == WS_HASH_TOMB) {
      stripe->hash[i] = idx;
      return;
    }
  }
...
void ws_clients_init(int max_clients) {
  ...
  n = g_clients_cap < WS_STRIPE_COUNT_MAX ? g_clients_cap : WS_STRIPE_COUNT_MAX;
  hash_cap = (uint32_t)next_pow2((size_t)g_clients_cap * 2u);
  for (int si = 0; si < n; si++) {
    ws_stripe_t *s = &g_stripes[si];
    s->hash_cap = hash_cap;
  ...
  if (g_free_slots) {
    for (int i = 0; i < g_clients_cap; i++) g_free_slots[i] = i;
