...
static void playlist_item_open_static(channel_item_t *it, int insecure_tls) {
  if (!strncmp(it->uri, "srt://", 6)) {
    char host[256];
    unsigned port;
    if (!route_parse_srt_uri(it->uri, host, sizeof host, &port)) it->static_ctx = capture_open_srt(host, port);
...
void build_from_xml(channel_list_t *l, const char *path) {
  ...
  free(xml);
  if (count == 0) log_line(TOOL_NAME ": --xml %s: no services found, list left empty", path);
}
...
void build_from_sds(channel_list_t *l, const char *addrport, const char *iface, double timeout_s) {
  ...
  dvbstp_reasm_free(r);
  mcast_close(m);
  if (total == 0) log_line(TOOL_NAME ": --sds %s: no services discovered in %.0fs", addrport, timeout_s);
}
